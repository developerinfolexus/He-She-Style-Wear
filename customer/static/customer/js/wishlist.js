...
function getWishlistKey() {
    const key = (window.getStorageKey && typeof window.getStorageKey === 'function') ? window.getStorageKey(WISHLIST_KEY) : WISHLIST_KEY;
    return key;
}
function getCartKey() {
...
function getWishlist() {
    try {
        ...
        const filtered = Array.isArray(list) ? list.filter(item => item && item.id) : [];
        return filtered;
    } catch (e) {
...
function setupRegionFlagDropdown() { /* ... (keep as before, dispatches 'regionChanged') ... */
    ...
    rDrop.querySelectorAll('.region-option').forEach(opt => {
        opt.addEventListener('click', e => {
            const newR = opt.getAttribute('data-value'); if (EXCHANGE_RATES[newR]) {
                setSelectedRegion(newR); updateFlag(newR); rCont.classList.remove('active');
...
// --- Cleanup function to remove invalid wishlist entries ---
function cleanupWishlist() {
    try {
        ...
        // Validate each item - must have id, name, image, and priceUSD
        const validItems = list.filter(item => {
            return item &&
                item.id &&
                item.name &&
                item.image &&
                (item.priceUSD !== undefined || item.price !== undefined);
        });

        // If we filtered out any items, save the cleaned list
        if (validItems.length !== list.length) {
            localStorage.setItem(key, JSON.stringify(validItems));
            console.log(`Cleaned wishlist: removed ${list.length - validItems.length} invalid items`);
        }
    } catch (e) {
        console.error("Error cleaning wishlist:", e);
    }
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Function to initialize wishlist after user is identified
    function initializeWishlist() {
        if (hasRendered) return; // Prevent double rendering
        hasRendered = true;

        console.log("Initializing wishlist with user context:", window.currentUserId);

        // If user is logged in, ensure guest wishlist is cleared
        if (window.currentUserId) {
            const guestKey = WISHLIST_KEY; // "he_she_wishlist"
            const userKey = getWishlistKey(); // "he_she_wishlist_USERID"

            // Clear the guest wishlist to prevent conflicts
            const guestData = localStorage.getItem(guestKey);
            if (guestData && guestData !== '[]') {
                console.log("Clearing guest wishlist to prevent conflicts");
                localStorage.setItem(guestKey, JSON.stringify([]));
            }
        }

        cleanupWishlist(); // Clean up invalid entries first
        renderWishlistLocal(); // Initial render

        // Hide loading overlay and show content smoothly
        const loadingOverlay = document.getElementById('wishlist-loading-overlay');
    ...
    // Wait for header to load before rendering wishlist
    // This ensures window.currentUserId is set correctly
    document.addEventListener('headerLoaded', () => {
        console.log("Wishlist: Header loaded, user context available");
        headerLoaded = true;
        // Small delay to ensure user identification is complete
        setTimeout(() => {
            initializeWishlist();
        }, 200);
    });

    // Fallback: If header takes too long or doesn't fire event, render after 3 seconds
    setTimeout(() => {
        if (!headerLoaded) {
            console.warn("Wishlist: Header load timeout, rendering with available context");
            initializeWishlist();
        }
    }, 3000);

    // --- Clear All button handler with custom dialog ---
    const clearBtn = document.getElementById('clear-wishlist-btn');
    const dialog = document.getElementById('clear-wishlist-dialog');
    const confirmBtn = document.getElementById('confirm-clear-btn');
    const cancelBtn = document.getElementById('cancel-clear-btn');

    if (clearBtn && dialog && confirmBtn && cancelBtn) {
        // Show dialog when Clear All is clicked
        ...
        // Clear wishlist when Confirm is clicked
        confirmBtn.addEventListener('click', () => {
            const key = getWishlistKey();
            console.log("Clearing wishlist with key:", key);
            localStorage.setItem(key, JSON.stringify([]));
            updateWishlistCount();
            renderWishlistLocal();
            dialog.style.display = 'none';

            // Show success message (optional - you can remove this if you don't want any notification)
            // Or replace with a toast notification
        });
    ...
    document.addEventListener('regionChanged', () => {
        console.log("Wishlist: Region changed, re-rendering");
        renderWishlistLocal();
    });
